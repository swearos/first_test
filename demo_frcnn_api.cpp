...
int main(int argc, char **argv) {
    ...
    for (size_t index = 0; index < images.size(); ++index) {
        ...
        for (int label = 0; label < FrcnnParam::n_classes; label++) {
            std::vector<BBox<float> > cur_res;
            ...
            vis_detections(ori, cur_res, LoadVocClass());
            std::string name = out_dir + images[index];
            char xx[100];
            sprintf(xx, "%s_%s.jpg", name.c_str(),
                    GetClassName(LoadVocClass(), label).c_str());
